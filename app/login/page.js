...
const Page = () => {
  ...
  useEffect(() => {
    document.title = "Login - FundBridge"
    if (session) {
      router.push("/dashboard");
    }
  }, [session, router]);

  // Render the login buttons if not logged in
  if (session) return null;

  const handleLogin = async () => {
    // await signOut(); // Ensure full logout and redirect to home
  
    setTimeout(() => {
      signIn("github", { callbackUrl: "/dashboard", prompt: "login" })
    }, 1000);
  };
  ...
  return (
    ...
    <div className='text-white text-center font-bold text-lg pt-2'>
      Login to Get your fans to support you
      <div className='flex flex-col items-center mt-10 gap-3'>

      <button onClick={handleLoginGoogle} type="button" className="text-white bg-[#4285F4] hover:bg-[#4285F4]/90 focus:ring-4 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 me-2 mb-2">
        ...
        <button onClick={handleLogin} type="button" className="text-white bg-[#24292F] hover:bg-[#24292F]/90 focus:ring-4 focus:outline-none focus:ring-[#24292F]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#050708]/30 me-2 mb-2">
